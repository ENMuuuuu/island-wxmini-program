...
class DatabaseStore:
    ...
    def get_home(self, db: Session, user_id: int) -> HomePageData:
        ...
        if today_habits:
            first = today_habits[0]
            featured = PreviewResponse(
                habit_id=first.id,
                preview_url=first.growth_state.current_preview_url,
                updated_at=first.growth_state.last_grow_at,
            )
    ...
    def _habit_to_detail(self, db: Session, habit: Habit) -> HabitDetail:
        ...
        streak = self._calc_habit_streak(db, habit.id)
        growth_state_view = GrowthStateView(
            current_layer_count=state.current_layer_count,
            lit_layer_count=state.lit_layer_count,
            dark_layer_count=state.dark_layer_count,
            progress_percent=state.progress_percent,
        ...
        return HabitDetail(
            ...
            growth_state=growth_state_view,
        )
    ...
    def _calc_habit_streak(self, db: Session, habit_id: int) -> int:
        records = db.scalars(
            select(HabitDayRecord.record_date)
            .where(
                HabitDayRecord.habit_id == habit_id,
                HabitDayRecord.record_status == "completed",
            )
            .order_by(HabitDayRecord.record_date.desc())
        ).all()
        completed_dates = set(records)
    ...
    def repair_growth_layer(self, db: Session, user_id: int, habit_id: int) -> RepairLayerResult:
        habit = self._get_habit_owned_by_user(db, user_id, habit_id)
        state = self._get_growth_state_model(db, habit_id)
        if state.dark_layer_count <= 0:
            raise HTTPException(status_code=400, detail="No dark layers to repair")

        # Find the most recent dark layer and flip it back to normal
        dark_layer = db.scalar(
            select(HabitGrowthLayer)
            .where(
                HabitGrowthLayer.habit_id == habit_id,
                HabitGrowthLayer.render_state == "dark",
            )
            .order_by(HabitGrowthLayer.layer_no.desc())
        )
        if not dark_layer:
            raise HTTPException(status_code=400, detail="No dark layers to repair")

        dark_layer.render_state = "normal"
        state.dark_layer_count = max(0, state.dark_layer_count - 1)
        state.lit_layer_count = state.lit_layer_count + 1
        total_layers = habit.theme.total_layers if habit.theme else 1
        state.progress_percent = (
            Decimal(state.lit_layer_count) / Decimal(total_layers) * Decimal("100")
        ).quantize(Decimal("0.01"))
        db.commit()

        return RepairLayerResult(
            repaired_layer=self._layer_to_view(dark_layer),
            growth_state=GrowthStateView(
                current_layer_count=state.current_layer_count,
                lit_layer_count=state.lit_layer_count,
                dark_layer_count=state.dark_layer_count,
